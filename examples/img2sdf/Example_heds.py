...
IMAGE_PATH = HERE / "HEDS" / "HEDS.jpg"
JSON_PATH  = HERE / "HEDS" / "HEDS.json"
...
# Build directly from the phi we already have
...
geom.save_png(
    OUT_DIR / "HEDS_phi.png",
    bounds=bounds,
    resolution=(512, 512),
    title="HEDS — Φ level-set  (blue=inside  red=outside)",
)
...
geom_union.save_png(
    OUT_DIR / "HEDS_union_translated.png",
    bounds=((0.0, float(w) + tx), (0.0, float(h))),
    resolution=(512, 512),
    title="HEDS — union(Φ, translate(Φ, 5%))",
)
...
geom_intersect.save_png(
    OUT_DIR / "HEDS_intersect_translated.png",
    bounds=bounds,
    resolution=(512, 512),
    title="HEDS — intersect(Φ, translate(Φ, 5%))",
)
