...
# Import the new standardized visualization function
try:
...
def save_3d_html_nato(values, name, bounds, out_dir="outputs/vis3d_plotly"):
    """
    Wrapper function using the new standardized save_levelset_html.

    This replaces the old custom visualization code with the new
    standardized visualization function from sdf3d.
    """
    if not HAS_VIZ:
        return
    ...
    # Use the new standardized save_levelset_html function
    try:
        save_levelset_html(values, bounds=bounds, filename=out_path)
        print(f"  ✅ Visualization: {out_path}")
    except Exception as e:
        print(f"  ⚠️  {name}: Error creating visualization - {e}")
        print(f"      Data range: [{values.min():.6f}, {values.max():.6f}]")
...
def create_fragment_geometry(lib):
    ...
    cylinder_box = Box(
        half_size=[fragment_radius * 2, cylinder_height / 2, fragment_radius * 2]
    )
    ...
    def capped_cone_sdf(p):
        ...
        return sdf.sdCappedCone(
            p_flipped, cone_half_height, tip_radius, fragment_radius
        )
...
def create_target_geometry(lib):
    ...
    print(
        f"    Center: ({target_center[0]*1000:.1f}, {target_center[1]*1000:.1f}, {target_center[2]*1000:.1f}) mm"
    )
...
def position_fragment_for_impact(
    fragment_geom, impact_angle_deg=0.0, distance_from_target=0.1
):
    """
...
def main():
    ...
    try:
        ...
        solid_mf, fragment_positioned_mf = create_full_domain(
            lib, fragment_positioned, target_mf
        )
        ...
        if HAS_VIZ:
            ...
            save_3d_html_nato(fragment_array, "nato_fragment", bounds)

            # Visualize target alone
            target_array = gather_multifab_to_array(target_mf, (n, n, n))
            save_3d_html_nato(target_array, "nato_target", bounds)

            # Visualize positioned fragment
            fragment_pos_array = gather_multifab_to_array(
                fragment_positioned_mf, (n, n, n)
            )
            save_3d_html_nato(fragment_pos_array, "nato_fragment_positioned", bounds)

            # Visualize full domain (fragment + target)
            solid_array = gather_multifab_to_array(solid_mf, (n, n, n))
            save_3d_html_nato(solid_array, "nato_full_domain", bounds)
        ...
        try:
            ...
            amr.write_single_level_plotfile(
                plotfile_name, solid_mf, varnames, geom, 0.0, 0  # time  # level
            )
