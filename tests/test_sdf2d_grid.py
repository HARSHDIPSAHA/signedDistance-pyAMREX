"""Tests for SDF2D.to_array() and save_npy()."""
...
class TestSampleLevelset2D:
    def test_output_shape(self):
        g = Circle2D(0.3)
        phi = g.to_array(((-1, 1), (-1, 1)), (32, 32))
        assert phi.shape == (32, 32)

    def test_non_square(self):
        g = Circle2D(0.3)
        phi = g.to_array(((-1, 1), (-1, 1)), (16, 32))
    ...
    def test_inside_negative_outside_positive(self):
        g = Circle2D(0.3)
        phi = g.to_array(((-1, 1), (-1, 1)), (64, 64))
        assert (phi < 0).any()
        assert (phi > 0).any()

    def test_dtype_is_float(self):
        g = Circle2D(0.3)
        phi = g.to_array(((-1, 1), (-1, 1)), (16, 16))
